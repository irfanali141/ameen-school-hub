...
function MeetingMinutes({addData}){
  ...
  useEffect(()=>{ const load=async()=>{ const {data}=await supabase.from("meetings").select("*").order("created_at",{ascending:false}).limit(30); setMeetings(data||[]); }; load(); },[]);
  ...
  const types={staff:{c:C.abuBakr,bg:"#dbeafe",i:"👨‍🏫",l:"Staff"},parents:{c:C.green,bg:"#dcfce7",i:"👪",l:"Parents"},board:{c:C.gold,bg:C.goldLight,i:"🏛️",l:"Board"},emergency:{c:C.red,bg:"#fee2e2",i:"🚨",l:"Emergency"},academic:{c:C.purple,bg:"#ede9fe",i:"📚",l:"Academic"}};
  ...
  return <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#0f172a 0%,#0d1f3c 50%,#0a1628 100%)",padding:"24px 20px",fontFamily:"'Public Sans',sans-serif",direction:"ltr"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"24px",flexWrap:"wrap",gap:"12px"}}>
      <div style={{display:"flex",alignItems:"center",gap:"12px"}}>
        ...
        <div><h2 style={{margin:0,fontSize:"1.4rem",fontWeight:"800",color:"#f1f5f9"}}>Meeting Minutes</h2><p style={{margin:0,fontSize:"0.72rem",color:"rgba(212,175,55,0.7)"}}>Meeting record and decisions</p></div>
      </div>
      <button style={{display:"flex",alignItems:"center",gap:"8px",padding:"10px 20px",borderRadius:"12px",border:"1px solid #d4af37",background:show?"rgba(212,175,55,0.15)":"transparent",color:"#d4af37",fontWeight:"700",fontSize:"0.82rem",cursor:"pointer",fontFamily:"'Public Sans',sans-serif"}} onClick={()=>setShow(!show)}><span className="material-symbols-rounded" style={{fontSize:"18px"}}>{show?"close":"add_circle"}</span>{show?"Cancel":"New Meeting"}</button>
    </div>
    {show&&<div style={{...glass,padding:"24px",marginBottom:"20px",borderColor:"rgba(212,175,55,0.3)"}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px",marginBottom:"12px"}}>
        <div style={{gridColumn:"1/-1"}}><label style={lbl}>Meeting Title *</label><input style={inp} value={f.title} onChange={e=>setF({...f,title:e.target.value})} placeholder="Monthly Staff Meeting..."/></div>
        ...
        <div style={{gridColumn:"1/-1"}}><label style={lbl}>Attendees</label><input style={inp} value={f.attendees} onChange={e=>setF({...f,attendees:e.target.value})} placeholder="List of names..."/></div>
        <div style={{gridColumn:"1/-1"}}><label style={lbl}>Agenda</label><textarea style={{...inp,minHeight:"60px",resize:"vertical"}} value={f.agenda} onChange={e=>setF({...f,agenda:e.target.value})} placeholder="Meeting agenda points..."/></div>
        <div style={{gridColumn:"1/-1"}}><label style={lbl}>Minutes</label><textarea style={{...inp,minHeight:"80px",resize:"vertical"}} value={f.minutes} onChange={e=>setF({...f,minutes:e.target.value})} placeholder="Full meeting minutes..."/></div>
        <div style={{gridColumn:"1/-1"}}><label style={lbl}>Decisions</label><textarea style={{...inp,minHeight:"60px",resize:"vertical"}} value={f.decisions} onChange={e=>setF({...f,decisions:e.target.value})} placeholder="Key decisions and instructions..."/></div>
      </div>
      <button style={{background:"linear-gradient(135deg,#d4af37,#b8960a)",color:N,border:"none",borderRadius:"12px",padding:"11px 26px",fontSize:"0.82rem",cursor:"pointer",fontFamily:"'Public Sans',sans-serif",fontWeight:"700"}} onClick={add}>Save Minutes</button>
    </div>}
