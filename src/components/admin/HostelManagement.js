...
function HostelManagement({students,addData}){
  ...
  useEffect(()=>{ const load=async()=>{ const {data:r}=await supabase.from("hostel_rooms").select("*"); setRooms(r||[]); const {data:res}=await supabase.from("hostel_residents").select("*"); setResidents(res||[]); }; load(); },[]);
  ...
  return <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#0f172a 0%,#0d1f3c 50%,#0a1628 100%)",padding:"24px 20px",fontFamily:"'Public Sans',sans-serif",direction:"ltr"}}>
    ...
    {show&&<div style={{...glass,padding:"24px",marginBottom:"20px",borderColor:"rgba(212,175,55,0.3)"}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px",marginBottom:"12px"}}>
        ...
        <div><label style={lbl}>منزل</label><select style={inp} value={f.floor} onChange={e=>setF({...f,floor:e.target.value})}>{[["Ground","گراؤنڈ"],["First","پہلی"],["Second","دوسری"],["Third","تیسری"]].map(([v,l])=><option key={v} value={v} style={{background:N2}}>{l}</option>)}</select></div>
        ...
        <div><label style={lbl}>قسم</label><select style={inp} value={f.type} onChange={e=>setF({...f,type:e.target.value})}><option value="standard" style={{background:N2}}>معیاری</option><option value="ac" style={{background:N2}}>ایئرکنڈیشنڈ</option><option value="vip" style={{background:N2}}>وی آئی پی</option></select></div>
      </div>
    ...
    <div style={{display:"flex",gap:"8px",marginBottom:"16px"}}>{[["rooms","🏠 Rooms"],["residents","👥 Residents"],["assign","➕ Admission"]].map(([t,l])=><button key={t} onClick={()=>setTab(t)} style={{padding:"8px 14px",borderRadius:"10px",border:tab===t?"none":"1px solid rgba(255,255,255,0.1)",cursor:"pointer",fontSize:"0.65rem",fontWeight:tab===t?"700":"400",background:tab===t?"linear-gradient(135deg,#d4af37,#b8960a)":"rgba(255,255,255,0.04)",color:tab===t?"#0f172a":"rgba(255,255,255,0.6)",fontFamily:"inherit"}}>{l}</button>)}</div>
    {tab==="rooms"&&<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:"14px"}}>
      {rooms.map(r=>{ const occ=getOccupancy(r.id); const full=occ>=r.capacity; return <div key={r.id} style={{...glass,padding:"18px",borderTop:`3px solid ${full?"#f87171":occ>0?"#fb923c":"#4ade80"}`}}>
        ...
        <div style={{fontSize:"0.6rem",color:"rgba(255,255,255,0.4)",marginTop:"4px"}}>{occ}/{r.capacity} residents</div>
      </div>; })}
    ...
    {tab==="assign"&&<div style={{...glass,padding:"24px"}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px",marginBottom:"12px"}}>
        <div><label style={lbl}>طالب علم *</label><select style={inp} value={resF.studentId} onChange={e=>setResF({...resF,studentId:e.target.value})}><option value="" style={{background:N2}}>-- منتخب کریں --</option>{students.map(s=><option key={s.id} value={s.id} style={{background:N2}}>{sLabel(s)}</option>)}</select></div>
    ...
    {tab==="residents"&&<div style={{...glass,overflow:"hidden"}}><div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse"}}>
      <thead><tr style={{background:"rgba(255,255,255,0.04)"}}>{["طالب علم","کمرہ","چیک ان","ماہانہ فیس","حیثیت"].map(h=><th key={h} style={{padding:"12px 14px",textAlign:"left",fontSize:"0.65rem",color:"rgba(212,175,55,0.8)",fontWeight:"700"}}>{h}</th>)}</tr></thead>
