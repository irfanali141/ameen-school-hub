...
function LearningMaterials({teachers,addData}){
  ...
  useEffect(()=>{ const load=async()=>{ const {data}=await supabase.from("learning_materials").select("*").order("created_at",{ascending:false}).limit(100); setMaterials(data||[]); }; load(); },[]);
  ...
  const types={pdf:{c:C.red,bg:"#fee2e2",i:"📄",l:"PDF"},video:{c:C.abuBakr,bg:"#dbeafe",i:"🎬",l:"Video"},audio:{c:C.purple,bg:"#ede9fe",i:"🎵",l:"Audio"},link:{c:C.teal,bg:"#ccfbf1",i:"🔗",l:"Link"},quiz:{c:C.amber,bg:"#fef3c7",i:"❓",l:"Quiz"},assignment:{c:C.gold,bg:C.goldLight,i:"📝",l:"Assignment"}};
  ...
  return <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#0f172a 0%,#0d1f3c 50%,#0a1628 100%)",padding:"24px 20px",fontFamily:"'Public Sans',sans-serif",direction:"ltr"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"24px",flexWrap:"wrap",gap:"12px"}}>
      <div style={{display:"flex",alignItems:"center",gap:"12px"}}>
        ...
        <div><h2 style={{margin:0,fontSize:"1.4rem",fontWeight:"800",color:"#f1f5f9"}}>Learning Materials</h2><p style={{margin:0,fontSize:"0.72rem",color:"rgba(212,175,55,0.7)"}}>Downloads, Videos, Assignments</p></div>
      </div>
      <button style={{display:"flex",alignItems:"center",gap:"8px",padding:"10px 20px",borderRadius:"12px",border:"1px solid #d4af37",background:show?"rgba(212,175,55,0.15)":"transparent",color:"#d4af37",fontWeight:"700",fontSize:"0.82rem",cursor:"pointer",fontFamily:"'Public Sans',sans-serif"}} onClick={()=>setShow(!show)}><span className="material-symbols-rounded" style={{fontSize:"18px"}}>{show?"close":"add_circle"}</span>{show?"Cancel":"New Material"}</button>
    </div>
    ...
    {show&&<div style={{...glass,padding:"24px",marginBottom:"20px",borderColor:"rgba(212,175,55,0.3)"}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px",marginBottom:"12px"}}>
        <div style={{gridColumn:"1/-1"}}><label style={lbl}>Title *</label><input style={inp} value={f.title} onChange={e=>setF({...f,title:e.target.value})} placeholder="Material name..."/></div>
        ...
        <div><label style={lbl}>Grade</label><input style={inp} value={f.grade} onChange={e=>setF({...f,grade:e.target.value})} placeholder="8..."/></div>
        ...
        <div style={{gridColumn:"1/-1"}}><label style={lbl}>Link</label><input style={{...inp,direction:"ltr"}} value={f.link} onChange={e=>setF({...f,link:e.target.value})} placeholder="https://..."/></div>
        <div style={{gridColumn:"1/-1"}}><label style={lbl}>Details</label><textarea style={{...inp,minHeight:"60px",resize:"vertical"}} value={f.description} onChange={e=>setF({...f,description:e.target.value})} placeholder="Short Details..."/></div>
      </div>
      <button style={{background:"linear-gradient(135deg,#d4af37,#b8960a)",color:N,border:"none",borderRadius:"12px",padding:"11px 26px",fontSize:"0.82rem",cursor:"pointer",fontFamily:"'Public Sans',sans-serif",fontWeight:"700"}} onClick={add}>Add</button>
    </div>}
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:"14px"}}>
      {filtered.map(m=>{ const tc=types[m.type]||types.link; const t=teachers.find(x=>x.id===m.teacherId); return <div key={m.id} style={{...glass,padding:"18px",borderTop:`3px solid ${tc.c}`}}>
        ...
        {m.subject&&<div style={{fontSize:"0.62rem",color:"rgba(255,255,255,0.5)",marginBottom:"4px"}}>📖 {m.subject}{m.grade&&` • Grade ${m.grade}`}</div>}
