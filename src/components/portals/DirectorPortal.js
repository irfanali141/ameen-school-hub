...
function DirectorPortal({students,teachers,houses,fees,results,hvsLogs}){
  ...
  return <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#0f172a 0%,#0d1f3c 50%,#0a1628 100%)",padding:"24px 20px",fontFamily:"'Public Sans',sans-serif",direction:"ltr"}}>
    <div style={{display:"flex",alignItems:"center",gap:"12px",marginBottom:"24px"}}>
      ...
      <div><h2 style={{margin:0,fontSize:"1.4rem",fontWeight:"800",color:"#f1f5f9"}}>Director Portal</h2><p style={{margin:0,fontSize:"0.72rem",color:"rgba(212,175,55,0.7)"}}>Full Institute — Overview</p></div>
    </div>
    <div style={{background:"linear-gradient(135deg,#0d1f3c,#0a1628)",borderRadius:"22px",padding:"24px",marginBottom:"24px",border:"1px solid rgba(212,175,55,0.2)"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:"16px"}}>
        {[{i:"🎓",n:students.length,l:"Students"},{i:"👨‍🏫",n:teachers.length,l:"Teachers"},{i:"📊",n:`${avgResult}%`,l:"Average Result"},{i:"💰",n:`${totalFees>0?Math.round((paidFees/totalFees)*100):0}%`,l:"Fee Collected"},{i:"🏆",n:winnerInfo?.nameEn||"—",l:"Super House"},{i:"📈",n:hvsLogs.length,l:"HVS Logs"}].map((x,i)=><div key={i} style={{textAlign:"center"}}><div style={{fontSize:"1.6rem"}}>{x.i}</div><div style={{fontSize:"1.4rem",fontWeight:"900",color:"#d4af37",marginTop:"4px"}}>{x.n}</div><div style={{fontSize:"0.58rem",color:"rgba(255,255,255,0.5)",marginTop:"2px"}}>{x.l}</div></div>)}
      </div>
    </div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"20px",marginBottom:"20px"}}>
      <div style={{...glass,padding:"20px"}}>
        <div style={{fontSize:"0.85rem",fontWeight:"700",color:"#d4af37",marginBottom:"16px"}}>🏆 House Ranking</div>
      ...
      <div style={{...glass,padding:"20px"}}>
        <div style={{fontSize:"0.85rem",fontWeight:"700",color:"#d4af37",marginBottom:"16px"}}>💰 Financial Summary</div>
        <div style={{marginBottom:"16px"}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:"6px"}}><span style={{fontSize:"0.68rem",color:"rgba(255,255,255,0.5)"}}>Total Fee</span><span style={{fontSize:"0.72rem",fontWeight:"700",color:"#f1f5f9"}}>Rs. {totalFees.toLocaleString()}</span></div>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:"6px"}}><span style={{fontSize:"0.68rem",color:"rgba(255,255,255,0.5)"}}>Received</span><span style={{fontSize:"0.72rem",fontWeight:"700",color:"#4ade80"}}>Rs. {paidFees.toLocaleString()}</span></div>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:"10px"}}><span style={{fontSize:"0.68rem",color:"rgba(255,255,255,0.5)"}}>Pending</span><span style={{fontSize:"0.72rem",fontWeight:"700",color:"#f87171"}}>Rs. {(totalFees-paidFees).toLocaleString()}</span></div>
          {pBar(paidFees,totalFees||1,"#4ade80")}
        </div>
        <div style={{background:"rgba(255,255,255,0.04)",borderRadius:"12px",padding:"12px",border:"1px solid rgba(255,255,255,0.06)"}}><div style={{fontSize:"0.65rem",color:"rgba(255,255,255,0.5)",marginBottom:"4px"}}>Collection Rate</div><div style={{fontSize:"2rem",fontWeight:"900",color:totalFees>0&&(paidFees/totalFees)>=0.8?"#4ade80":"#fb923c"}}>{totalFees>0?Math.round((paidFees/totalFees)*100):0}%</div></div>
      </div>
    </div>
    <div style={{...glass,padding:"20px"}}>
      <div style={{fontSize:"0.85rem",fontWeight:"700",color:"#d4af37",marginBottom:"16px"}}>📊 Results Analysis</div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:"12px"}}>
        {[["A+",results.filter(r=>r.grade==="A+").length,"#4ade80"],["A",results.filter(r=>r.grade==="A").length,"#4ade80"],["B",results.filter(r=>r.grade==="B").length,"#60a5fa"],["C",results.filter(r=>r.grade==="C").length,"#fb923c"],["D",results.filter(r=>r.grade==="D").length,"#fb923c"],["F",results.filter(r=>r.grade==="F").length,"#f87171"]].map(([g,n,c])=><div key={g} style={{background:`${c}12`,borderRadius:"12px",padding:"12px",textAlign:"center",border:`1px solid ${c}25`}}><div style={{fontSize:"1.2rem",fontWeight:"900",color:c}}>{g}</div><div style={{fontSize:"1.4rem",fontWeight:"900",color:c}}>{n}</div><div style={{fontSize:"0.6rem",color:"rgba(255,255,255,0.5)"}}>Students</div></div>)}
      </div>
    </div>
  </div>;
}
