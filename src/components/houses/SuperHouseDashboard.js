...
import { C, S, hBadge, pBar, HOUSES, HVS_TOTAL, sLabel } from "../../constants";
...
function SuperHouseDashboard({houses,hvsLogs,students}){
  ...
  const houseStats=HOUSES.map(h=>{ const hd=houses.find(x=>x.id===h.id)||{}; const studs=students.filter(s=>s.houseId===h.id); const hLogs=hvsLogs.filter(l=>l.houseId===h.id); const avgHvs=hLogs.length>0?Math.round(hLogs.reduce((s,l)=>s+(l.totalScore||l.total_score||0),0)/hLogs.length):0; const bestWeek=hLogs.length>0?Math.max(...hLogs.map(l=>l.total||0)):0; const rank=sorted.findIndex(x=>x.id===h.id)+1; return {...h,...hd,studs,hLogs,avgHvs,bestWeek,rank}; });
  ...
  return <div style={S.page}>
    ...
    <div style={{fontSize:"0.62rem",color:"#888",marginBottom:"20px"}}>تفصیلی ہاؤس تجزیہ اور کارکردگی</div>
    {winner&&winnerInfo&&<div style={{background:winnerInfo.gradient,borderRadius:"22px",padding:"28px",marginBottom:"24px",color:C.white,textAlign:"center",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{fontSize:"0.7rem",opacity:0.8,letterSpacing:"0.2em",marginBottom:"6px"}}>سپر ہاؤس — موجودہ سرفہرست</div>
    ...
    {selectedStats&&selectedInfo&&<div className="hv-card" style={{...S.card,marginBottom:"20px",borderTop:`4px solid ${selectedInfo.color}`}}>
      <div style={{fontSize:"0.9rem",fontWeight:"700",color:C.navy,marginBottom:"16px"}}>{selectedInfo.emoji} {selectedInfo.nameEn} ہاؤس — تفصیلات</div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:"10px",marginBottom:"16px"}}>
        {[{l:"کل نمبر",v:selectedStats.points||0,c:selectedInfo.color},{l:"طلباء",v:selectedStats.studs?.length||0,c:C.abuBakr},{l:"ایچ وی ایس اندراج",v:selectedStats.hLogs?.length||0,c:C.teal},{l:"اوسط ایچ وی ایس",v:`${selectedStats.avgHvs}/${HVS_TOTAL}`,c:C.amber},{l:"بہترین",v:selectedStats.bestWeek,c:C.green},{l:"درجہ",v:`#${selectedStats.rank}`,c:C.gold}].map((x,i)=><div key={i} style={{background:`${x.c}10`,borderRadius:"12px",padding:"12px",border:`1px solid ${x.c}20`,textAlign:"center"}}><div style={{fontSize:"1.2rem",fontWeight:"800",color:x.c}}>{x.v}</div><div style={{fontSize:"0.6rem",color:"#888",marginTop:"2px"}}>{x.l}</div></div>)}
      </div>
      <div style={{fontSize:"0.72rem",fontWeight:"700",color:C.navy,marginBottom:"10px"}}>👥 طلباء</div>
    ...
    <div style={S.card} className="hv-card">
     <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"16px"}}><div style={{fontSize:"0.85rem",fontWeight:"700",color:C.navy}}>📊 ہاؤس موازنہ</div><PrintBtn onClick={()=>printHVSReport(houses,hvsLogs,students)} label="HVS Report PDF"/></div>
      <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse"}}>
        <thead><tr><th style={S.th}>ہاؤس</th><th style={S.th}>درجہ</th><th style={S.th}>نمبر</th><th style={S.th}>طلباء</th><th style={S.th}>اوسط ایچ وی ایس</th><th style={S.th}>بہترین</th></tr></thead>
        <tbody>{houseStats.map((h,i)=>{ const info=HOUSES.find(x=>x.id===h.id)||{}; return <tr key={h.id} style={{background:i===0?`${info.color}08`:undefined}}>
          ...
          <td style={{...S.td,fontWeight:"800",color:i===0?C.gold:"#888"}}>{i===0?"👑 اول":`#${i+1}`}</td>
