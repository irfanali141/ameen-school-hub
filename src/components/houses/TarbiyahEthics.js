...
function TarbiyahEthics({students,addData}){
  ...
  useEffect(()=>{ getData("ethics_logs").then(data=>setLogs(data||[])); },[]);
  ...
  const cats={adab:{c:C.gold,bg:C.goldLight,i:"🌟",l:"Adab"},prayer:{c:C.purple,bg:"#ede9fe",i:"🕌",l:"Namaz"},quran:{c:C.green,bg:"#dcfce7",i:"📖",l:"Quran"},honesty:{c:C.abuBakr,bg:"#dbeafe",i:"✨",l:"Honesty"},discipline:{c:C.navy,bg:"#e0e7ff",i:"⚡",l:"Discipline"},helping:{c:C.teal,bg:"#ccfbf1",i:"🤝",l:"Cooperation"},misconduct:{c:C.red,bg:"#fee2e2",i:"⚠️",l:"Misconduct"}};
  ...
  return <div style={S.page}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"16px"}}>
      <div><div style={{fontSize:"1.1rem",fontWeight:"700",color:C.navy}}>🌟 Training Ethics Monitor</div><div style={{fontSize:"0.62rem",color:"#888",marginTop:"2px"}}>Character, Adab & Ethics Record</div></div>
    ...
    {show&&<div className="hv-card" style={{...S.card,marginBottom:"20px",background:`linear-gradient(135deg,${C.goldLight},#fdf8ee)`,border:`2px solid ${C.gold}30`}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px",marginBottom:"12px"}}>
        ...
        <div><label style={{fontSize:"0.62rem",color:"#888",marginBottom:"4px",display:"block"}}>Type</label><select style={S.inpSm} value={f.type} onChange={e=>setF({...f,type:e.target.value})}><option value="positive">✅ Positive</option><option value="negative">❌ Negative</option></select></div>
        <div><label style={{fontSize:"0.62rem",color:"#888",marginBottom:"4px",display:"block"}}>Category</label><select style={S.inpSm} value={f.category} onChange={e=>setF({...f,category:e.target.value})}>{Object.entries(cats).map(([k,v])=><option key={k} value={k}>{v.i} {v.l}</option>)}</select></div>
        <div><label style={{fontSize:"0.62rem",color:"#888",marginBottom:"4px",display:"block"}}>Points</label><input style={{...S.inpSm,direction:"ltr"}} type="number" min="1" max="50" value={f.points} onChange={e=>setF({...f,points:e.target.value})}/></div>
        <div style={{gridColumn:"1/-1"}}><label style={{fontSize:"0.62rem",color:"#888",marginBottom:"4px",display:"block"}}>Details</label><textarea style={{...S.inpSm,minHeight:"60px",resize:"vertical"}} value={f.description} onChange={e=>setF({...f,description:e.target.value})} placeholder="Event details..."/></div>
      </div>
      <button style={{...S.saveBtn,background:f.type==="positive"?`linear-gradient(135deg,${C.green},#15803d)`:`linear-gradient(135deg,${C.red},#991b1b)`}} onClick={add}>{f.type==="positive"?"✅ Record Positive":"❌ Record Negative"}</button>
    </div>}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"20px"}}>
      <div style={S.card} className="hv-card">
        <div style={{fontSize:"0.85rem",fontWeight:"700",color:C.navy,marginBottom:"16px"}}>🏆 Ethics Leaderboard</div>
        {studentPoints.slice(0,10).map((s,i)=>{ const h=HOUSES.find(x=>x.id===s.houseId)||{}; return <div key={s.id} style={{display:"flex",alignItems:"center",gap:"10px",marginBottom:"10px",padding:"8px 10px",background:i===0?"linear-gradient(135deg,#fef3c7,#fff)":"#fafaf8",borderRadius:"10px"}}>
          ...
          <div style={{flex:1}}><div style={{fontSize:"0.7rem",fontWeight:"700",color:C.navy}}>{s.name}</div><div style={{fontSize:"0.55rem",color:"#888"}}>{s.logs} entries</div></div>
          <span style={{fontSize:"0.82rem",fontWeight:"900",color:s.points>=0?C.green:C.red}}>{s.points>0?"+":""}{s.points}</span>
        </div>; })}
        {studentPoints.length===0&&<div style={{textAlign:"center",color:"#bbb",padding:"20px"}} className="ur">کوئی ڈیٹا نہیں</div>}
      </div>
      <div style={S.card} className="hv-card">
        <div style={{fontSize:"0.85rem",fontWeight:"700",color:C.navy,marginBottom:"16px"}}>📋 Historical Entries</div>
        ...
        {logs.length===0&&<div style={{textAlign:"center",color:"#bbb",padding:"20px"}} className="ur">ابھی کوئی اندراج نہیں</div>}
      </div>
    </div>
  </div>;
}
