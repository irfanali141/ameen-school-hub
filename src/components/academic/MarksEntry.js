...
function MarksEntry({students,addData}){
  ...
  useEffect(()=>{ getData("assessments").then(e=>setExams(e||[])); getData("assessment_marks").then(m=>setMarks(m||[])); },[]);
  const addExam=async()=>{ if(!f.title)return; await addData("assessments",{...f,totalMarks:Number(f.totalMarks)}); setShow(false); setCustomTitle(false); setF({title:"",subject:"",grade:"",date:"",totalMarks:100,type:"monthly"}); getData("assessments").then(e=>setExams(e||[])); };
  ...
  const types={monthly:{c:"#60a5fa",l:"Monthly"},midterm:{c:"#fb923c",l:"Mid-Term"},annual:{c:G,l:"Annual"},unit:{c:"#4ade80",l:"Unit"},quiz:{c:"#a78bfa",l:"Quiz"}};
    ...
    <div style={{minHeight:"100vh",background:`linear-gradient(160deg,${N} 0%,#0d1f3c 50%,#0a1628 100%)`,padding:"24px 20px",fontFamily:"'Public Sans',sans-serif",direction:"ltr"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"28px",flexWrap:"wrap",gap:"12px"}}>
        <div style={{display:"flex",alignItems:"center",gap:"12px"}}>
          ...
          <div>
            <h1 style={{margin:0,fontSize:"1.5rem",fontWeight:"800",color:"#f1f5f9",letterSpacing:"-0.5px"}}>Marks Entry</h1>
      ...
      {show&&(
        <div style={{...glass,padding:"24px",marginBottom:"24px",borderColor:"rgba(212,175,55,0.3)"}}>
          <div style={{display:"flex",alignItems:"center",gap:"10px",marginBottom:"20px"}}>
            ...
            <span style={{color:G,fontWeight:"700",fontSize:"1rem"}}>New Exam Entry</span>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"14px",marginBottom:"16px"}}>
            ...
            <div><label style={lbl}>Subject</label><select style={inp} value={f.subject} onChange={e=>setF({...f,subject:e.target.value})}><option value="" style={{background:N2}}>-- Select Subject --</option>{SUBJECTS.map(s=><option key={s} value={s} style={{background:N2}}>{s}</option>)}</select></div>
            <div><label style={lbl}>Grade</label><select style={inp} value={f.grade} onChange={e=>setF({...f,grade:e.target.value})}>
              <option value="" style={{background:N2}}>-- All Grades --</option>
              {gradeOptions.map(g=><option key={g} value={g} style={{background:N2}}>{g}</option>)}
            </select></div>
            <div><label style={lbl}>Date</label><input style={{...inp,direction:"ltr"}} type="date" value={f.date} onChange={e=>setF({...f,date:e.target.value})}/></div>
            <div><label style={lbl}>Total Marks</label><input style={{...inp,direction:"ltr"}} type="number" value={f.totalMarks} onChange={e=>setF({...f,totalMarks:e.target.value})}/></div>
          ...
          <div style={{display:"flex",gap:"10px",justifyContent:"flex-end"}}>
            ...
            <button onClick={addExam} style={{display:"flex",alignItems:"center",gap:"8px",padding:"10px 24px",borderRadius:"10px",border:"none",background:`linear-gradient(135deg,${G},#b8960a)`,color:N,fontWeight:"700",fontSize:"0.82rem",cursor:"pointer",fontFamily:"'Public Sans',sans-serif"}}>
              <span className="material-symbols-rounded" style={{fontSize:"18px"}}>save</span>Exam Add
            </button>
          </div>
        </div>
      )}
      ...
      <div style={{display:"flex",gap:"6px",marginBottom:"24px",background:"rgba(255,255,255,0.04)",borderRadius:"12px",padding:"4px",width:"fit-content"}}>
        {[["exams","library_books","Exams"],["entry","edit","Marks Entry"]].map(([t,ic,l])=>(
            ...
            return (
              <div key={ex.id} className="hv-card" style={{...glass,padding:"20px",cursor:"pointer",borderTop:`3px solid ${tc.c}`,position:"relative",overflow:"hidden"}} onClick={()=>{setSelExam(ex);setTab("entry");}}>
                ...
                {avg!==null&&(
                  <div style={{marginBottom:"12px"}}>
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:"5px"}}>
                      <span style={{fontSize:"0.62rem",color:"rgba(241,245,249,0.4)"}}>Average Performance</span>
                ...
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                  <div style={{fontSize:"0.65rem",color:"rgba(212,175,55,0.6)"}}>{exMarks.length} students' marks</div>
                  <div style={{display:"flex",alignItems:"center",gap:"4px",fontSize:"0.65rem",color:`rgba(${tc.c.replace("#","").match(/.{2}/g).map(h=>parseInt(h,16)).join(",")},0.8)`}}>
                    <span className="material-symbols-rounded" style={{fontSize:"14px"}}>arrow_back</span>Marks Entry
                  </div>
                </div>
              </div>
            );
        ...
        <div>
          ...
          <div style={{...glass,padding:"16px 20px",marginBottom:"20px",display:"flex",gap:"12px",alignItems:"center",flexWrap:"wrap"}}>
            ...
            <select style={{...inp,flex:1,minWidth:"220px"}} value={selExam?.id||""} onChange={e=>setSelExam(exams.find(x=>x.id===e.target.value)||null)}>
              <option value="" style={{background:N2}}>-- Exam Select --</option>
          ...
          {!selExam&&(
            <div style={{...glass,padding:"60px 20px",textAlign:"center"}}>
              ...
              <div style={{color:"rgba(241,245,249,0.4)",fontSize:"0.9rem"}}>Select an exam above</div>
            </div>
          )}
            ...
            <div>
              ...
              <div style={{...glass,padding:"18px 20px",marginBottom:"16px",borderColor:`rgba(212,175,55,0.3)`,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:"12px"}}>
                <div>
                  ...
                  <div style={{fontSize:"0.72rem",color:"rgba(212,175,55,0.7)"}}>{selExam.subject||"—"}{selExam.grade?` • ${selExam.grade}`:""} • Total Marks: {selExam.totalMarks}</div>
                </div>
                <div style={{display:"flex",gap:"10px"}}>
                  <div style={{textAlign:"center"}}>
                    <div style={{fontSize:"0.6rem",color:"rgba(241,245,249,0.4)",marginBottom:"2px"}}>Complete</div>
              ...
              {selMarks.length>0&&(
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"12px",marginBottom:"20px"}}>
                  {[
                    {icon:"trending_up",label:"High Marks",val:`${statHigh}%`,color:"#4ade80"},
                    {icon:"show_chart",label:"Average",val:`${statAvg}%`,color:G},
                    {icon:"trending_down",label:"Low Marks",val:`${statLow}%`,color:"#f87171"},
                  ].map(s=>(
              ...
              <div style={{...glass,overflow:"hidden"}}>
                <div style={{padding:"16px 20px",borderBottom:"1px solid rgba(255,255,255,0.08)",display:"flex",alignItems:"center",gap:"8px"}}>
                  ...
                  <span style={{color:"#f1f5f9",fontWeight:"700",fontSize:"0.9rem"}}>Enter Marks</span>
                </div>
                <div style={{overflowX:"auto"}}>
                  <table style={{width:"100%",borderCollapse:"collapse"}}>
                    <thead>
                      <tr style={{background:"rgba(255,255,255,0.04)"}}>
                        <th style={{padding:"12px 20px",textAlign:"left",fontSize:"0.68rem",fontWeight:"700",color:"rgba(212,175,55,0.8)",borderBottom:"1px solid rgba(255,255,255,0.08)"}}>Student</th>
                        <th style={{padding:"12px 12px",textAlign:"center",fontSize:"0.68rem",fontWeight:"700",color:"rgba(212,175,55,0.8)",borderBottom:"1px solid rgba(255,255,255,0.08)",direction:"ltr"}}>Code</th>
                        <th style={{padding:"12px 20px",textAlign:"center",fontSize:"0.68rem",fontWeight:"700",color:"rgba(212,175,55,0.8)",borderBottom:"1px solid rgba(255,255,255,0.08)"}}>Marks / {selExam.totalMarks}</th>
                        <th style={{padding:"12px 20px",textAlign:"center",fontSize:"0.68rem",fontWeight:"700",color:"rgba(212,175,55,0.8)",borderBottom:"1px solid rgba(255,255,255,0.08)"}}>Percentage / Grade</th>
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        return (
                          <tr key={s.id} style={{borderBottom:"1px solid rgba(255,255,255,0.05)",background:existing?"rgba(74,222,128,0.04)":i%2===0?"rgba(255,255,255,0.02)":"transparent",transition:"background 0.15s"}}>
                            ...
                            <td style={{padding:"12px 20px",textAlign:"center"}}>
                              ...
                              ):(()=>{const over=bulkVal!==""&&Number(bulkVal)>selExam.totalMarks; return(<div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:"3px"}}>
                                <input
                                  type="number" min="0" max={selExam.totalMarks}
                                  placeholder="—"
                                  value={bulkVal}
                                  onChange={e=>setBulk({...bulk,[s.id]:e.target.value})}
                                  style={{width:"80px",padding:"8px 10px",borderRadius:"8px",border:`1px solid ${over?"rgba(248,113,113,0.7)":bulkVal?"rgba(212,175,55,0.5)":"rgba(255,255,255,0.12)"}`,background:over?"rgba(248,113,113,0.08)":"rgba(255,255,255,0.06)",color:over?"#f87171":"#f1f5f9",fontSize:"0.85rem",fontFamily:"monospace",outline:"none",textAlign:"center",direction:"ltr",colorScheme:"dark"}}
                                />
                                {over&&<span style={{fontSize:"0.55rem",color:"#f87171",whiteSpace:"nowrap"}}>max {selExam.totalMarks}</span>}
                              </div>);})()}
                            </td>
                            <td style={{padding:"12px 20px",textAlign:"center"}}>
                              {existing?(
                      ...
                      {classStudents.length===0&&(
                        <tr><td colSpan={4} style={{padding:"50px 20px",textAlign:"center",color:"rgba(241,245,249,0.3)",fontSize:"0.85rem"}}>
                          ...
                          <span className="ur">اس گریڈ میں کوئی طالب علم نہیں</span>
                        </td></tr>
                      )}
                    </tbody>
                  </table>
