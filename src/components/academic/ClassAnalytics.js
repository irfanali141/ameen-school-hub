...
function ClassAnalytics({students,results}){
  ...
  return <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#0f172a 0%,#0d1f3c 50%,#0a1628 100%)",padding:"24px 20px",fontFamily:"'Public Sans',sans-serif",direction:"ltr"}}>
    <div style={{display:"flex",alignItems:"center",gap:"12px",marginBottom:"24px"}}>
      ...
      <div><h2 style={{margin:0,fontSize:"1.4rem",fontWeight:"800",color:"#f1f5f9"}}>Class Analytics</h2></div>
    </div>
    <div>
    <div style={{display:"flex",gap:"8px",marginBottom:"20px",flexWrap:"wrap"}}>
      {[["all","📊 All"],...grades.map(g=>[g,`Grade ${g}`])].map(([g,l])=><button key={g} onClick={()=>setSelGrade(g)} style={{padding:"6px 14px",borderRadius:"20px",border:"none",cursor:"pointer",fontSize:"0.62rem",fontWeight:selGrade===g?"700":"400",background:selGrade===g?`linear-gradient(135deg,${C.gold},${C.goldDark})`:C.white,color:selGrade===g?C.white:"#888",fontFamily:"inherit"}}>{l}</button>)}
    </div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(150px,1fr))",gap:"12px",marginBottom:"20px"}}>
      {[{c:C.navy,i:"👥",n:filtered.length,l:"Students"},{c:overallAvg>=70?C.green:C.amber,i:"📈",n:`${overallAvg}%`,l:"Average"},{c:C.gold,i:"🏆",n:filtered.filter(s=>getAvg(s)>=80).length,l:"A Grade"},{c:C.red,i:"⚠️",n:filtered.filter(s=>{ const a=getAvg(s); return a<50&&results.some(r=>r.studentId===s.id); }).length,l:"Weak"}].map((x,i)=><div key={i} style={{background:`linear-gradient(135deg,${x.c}12,${x.c}05)`,borderRadius:"16px",padding:"16px",border:`2px solid ${x.c}20`,textAlign:"center"}}><div style={{fontSize:"1.3rem"}}>{x.i}</div><div style={{fontSize:"1.4rem",fontWeight:"900",color:x.c}}>{x.n}</div><div style={{fontSize:"0.6rem",color:"#888"}}>{x.l}</div></div>)}
    </div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"20px",marginBottom:"20px"}}>
      <div style={{...glass,padding:"20px"}}><div style={{fontSize:"0.85rem",fontWeight:"700",color:"#d4af37",marginBottom:"16px"}}>🏠 House Performance</div>
        {houseData.map(h=><div key={h.id} style={{marginBottom:"14px"}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:"4px"}}><span style={{fontSize:"0.72rem",fontWeight:"700",color:C.navy}}>{h.emoji} {h.nameEn}</span><span style={{fontSize:"0.68rem",fontWeight:"700",color:h.avg>=70?C.green:h.avg>=50?C.amber:C.red}}>{h.avg}%</span></div>{pBar(h.avg,100,h.avg>=70?C.green:h.avg>=50?C.amber:C.red)}<div style={{fontSize:"0.58rem",color:"#aaa",marginTop:"2px"}}>{h.count} Students</div></div>)}
      </div>
      <div style={S.card} className="hv-card"><div style={{fontSize:"0.85rem",fontWeight:"700",color:C.navy,marginBottom:"16px"}}>🏆 Top Students</div>
        ...
        {topStudents.length===0&&<div style={{textAlign:"center",color:"#bbb",padding:"20px"}} className="ur">کوئی ڈیٹا نہیں</div>}
      </div>
    </div>
    <div style={S.card} className="hv-card"><div style={{fontSize:"0.85rem",fontWeight:"700",color:C.navy,marginBottom:"16px"}}>📚 Subject-wise Analysis</div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:"12px"}}>
        {Object.entries(subjectData).map(([sub,data])=>{ const avg=Math.round(data.total/data.count); return <div key={sub} style={{background:"#fafaf8",borderRadius:"12px",padding:"12px"}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:"6px"}}><span style={{fontSize:"0.72rem",fontWeight:"700",color:C.navy}}>{sub}</span><span style={{fontSize:"0.65rem",fontWeight:"700",color:avg>=70?C.green:avg>=50?C.amber:C.red}}>{avg}%</span></div>{pBar(avg,100,avg>=70?C.green:avg>=50?C.amber:C.red)}<div style={{fontSize:"0.58rem",color:"#aaa",marginTop:"4px"}}>{data.count} Record</div></div>; })}
        {Object.keys(subjectData).length===0&&<div style={{textAlign:"center",color:"#bbb",padding:"20px",gridColumn:"1/-1"}} className="ur">کوئی ڈیٹا نہیں</div>}
      </div>
    </div>
  </div></div>;
}
