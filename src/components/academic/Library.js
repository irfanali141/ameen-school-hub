...
function Library({students,addData}){
  ...
  useEffect(()=>{ const load=async()=>{ const {data:b}=await supabase.from("library_books").select("*"); setBooks(b||[]); const {data:i}=await supabase.from("library_issues").select("*").order("created_at",{ascending:false}).limit(50); setIssues(i||[]); }; load(); },[]);
  ...
  const issueBook=async()=>{ if(!issueF.bookId||!issueF.studentId)return; await addData("library_issues",{...issueF,status:"issued",issuedDate:new Date().toISOString().split("T")[0]}); await supabase.from("library_books").update({available:Math.max(0,(books.find(b=>b.id===issueF.bookId)?.available||1)-1)}).eq("id",issueF.bookId); setIssueF({bookId:"",studentId:"",dueDate:""}); };
  const returnBook=async(issue)=>{ await supabase.from("library_issues").update({status:"returned",returned_date:new Date().toISOString().split("T")[0]}).eq("id",issue.id); await supabase.from("library_books").update({available:(books.find(b=>b.id===issue.bookId)?.available||0)+1}).eq("id",issue.bookId); };
  ...
  return <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#0f172a 0%,#0d1f3c 50%,#0a1628 100%)",padding:"24px 20px",fontFamily:"'Public Sans',sans-serif",direction:"ltr"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"24px",flexWrap:"wrap",gap:"12px"}}>
      <div style={{display:"flex",alignItems:"center",gap:"12px"}}>
        ...
        <div><h2 style={{margin:0,fontSize:"1.4rem",fontWeight:"800",color:"#f1f5f9"}}>Library</h2><p style={{margin:0,fontSize:"0.72rem",color:"rgba(212,175,55,0.7)"}}>{books.length} Books</p></div>
      </div>
      <button style={{display:"flex",alignItems:"center",gap:"8px",padding:"10px 20px",borderRadius:"12px",border:"1px solid #d4af37",background:show?"rgba(212,175,55,0.15)":"transparent",color:"#d4af37",fontWeight:"700",fontSize:"0.82rem",cursor:"pointer",fontFamily:"'Public Sans',sans-serif"}} onClick={()=>setShow(!show)}><span className="material-symbols-rounded" style={{fontSize:"18px"}}>{show?"close":"add_circle"}</span>{show?"Cancel":"New Book"}</button>
    </div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(150px,1fr))",gap:"12px",marginBottom:"20px"}}>
      {[{c:"#60a5fa",i:"library_books",n:books.length,l:"کل کتابیں"},{c:"#4ade80",i:"check_circle",n:issues.filter(i=>i.status==="issued").length,l:"جاری"},{c:"#f87171",i:"warning",n:overdue.length,l:"وقت گزر گیا"},{c:"#d4af37",i:"menu_book",n:books.reduce((s,b)=>s+(b.available||0),0),l:"دستیاب"}].map((x,i)=><div key={i} style={{...glass,padding:"16px",textAlign:"center"}}><span className="material-symbols-rounded" style={{fontSize:"28px",color:x.c,display:"block",marginBottom:"8px"}}>{x.i}</span><div style={{fontSize:"1.4rem",fontWeight:"900",color:x.c}}>{x.n}</div><div style={{fontSize:"0.6rem",color:"rgba(255,255,255,0.5)"}}>{x.l}</div></div>)}
    </div>
    {show&&<div style={{...glass,padding:"24px",marginBottom:"20px",borderColor:"rgba(212,175,55,0.3)"}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px",marginBottom:"12px"}}>
        ...
        <div><label style={lbl}>نسخے</label><input style={{...inp,direction:"ltr",colorScheme:"dark"}} type="number" value={f.totalCopies} onChange={e=>setF({...f,totalCopies:e.target.value})}/></div>
      </div>
      <button style={{background:"linear-gradient(135deg,#d4af37,#b8960a)",color:N,border:"none",borderRadius:"12px",padding:"11px 26px",fontSize:"0.82rem",cursor:"pointer",fontFamily:"'Public Sans',sans-serif",fontWeight:"700"}} onClick={addBook}>Save</button>
    </div>}
    <div style={{display:"flex",gap:"6px",marginBottom:"16px",background:"rgba(255,255,255,0.04)",padding:"4px",borderRadius:"10px",width:"fit-content"}}>{[["books","library_books","کتابیں"],["issue","upload","کتاب جاری کریں"],["issued","list_alt","جاری کتابیں"]].map(([t,ic,l])=><button key={t} onClick={()=>setTab(t)} style={{display:"flex",alignItems:"center",gap:"5px",padding:"8px 14px",borderRadius:"8px",border:"none",cursor:"pointer",fontSize:"0.65rem",fontWeight:tab===t?"700":"400",background:tab===t?"linear-gradient(135deg,#d4af37,#b8960a)":"transparent",color:tab===t?N:"rgba(255,255,255,0.5)",fontFamily:"inherit"}}><span className="material-symbols-rounded" style={{fontSize:"15px"}}>{ic}</span>{l}</button>)}</div>
    {tab==="books"&&<div style={{...glass,overflow:"hidden"}}><div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse"}}>
      <thead><tr style={{background:"rgba(255,255,255,0.04)"}}><th style={{padding:"12px 14px",textAlign:"left",fontSize:"0.65rem",color:"rgba(212,175,55,0.8)",fontWeight:"700"}}>کتاب</th><th style={{padding:"12px 14px",textAlign:"left",fontSize:"0.65rem",color:"rgba(212,175,55,0.8)",fontWeight:"700"}}>مصنف</th><th style={{padding:"12px 14px",textAlign:"left",fontSize:"0.65rem",color:"rgba(212,175,55,0.8)",fontWeight:"700"}}>زمرہ</th><th style={{padding:"12px 14px",textAlign:"left",fontSize:"0.65rem",color:"rgba(212,175,55,0.8)",fontWeight:"700"}}>کل</th><th style={{padding:"12px 14px",textAlign:"left",fontSize:"0.65rem",color:"rgba(212,175,55,0.8)",fontWeight:"700"}}>دستیاب</th></tr></thead>
    ...
    {tab==="issue"&&<div style={{...glass,padding:"24px",borderColor:"rgba(212,175,55,0.3)"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px",marginBottom:"12px"}}>
        ...
        <div><label style={lbl}>طالب علم *</label><select style={inp} value={issueF.studentId} onChange={e=>setIssueF({...issueF,studentId:e.target.value})}><option value="" style={{background:N2}}>-- منتخب کریں --</option>{students.map(s=><option key={s.id} value={s.id} style={{background:N2}}>{sLabel(s)}</option>)}</select></div>
        <div><label style={lbl}>واپسی کی تاریخ</label><input style={{...inp,direction:"ltr",colorScheme:"dark"}} type="date" value={issueF.dueDate} onChange={e=>setIssueF({...issueF,dueDate:e.target.value})}/></div>
      </div>
    ...
    {tab==="issued"&&<div style={{...glass,overflow:"hidden"}}><div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse"}}>
      <thead><tr style={{background:"rgba(255,255,255,0.04)"}}>{["طالب علم","کتاب","جاری تاریخ","واپسی","حیثیت","عمل"].map(h=><th key={h} style={{padding:"12px 14px",textAlign:"left",fontSize:"0.65rem",color:"rgba(212,175,55,0.8)",fontWeight:"700"}}>{h}</th>)}</tr></thead>
      ...
      </tr>; })}{issues.length===0&&<tr><td colSpan={6} style={{padding:"40px",textAlign:"center",color:"rgba(255,255,255,0.3)"}}><span className="ur">کوئی اندراج نہیں</span></td></tr>}</tbody>
    </table></div></div>}
  </div>;
}
