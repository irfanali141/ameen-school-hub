...
function DMC({students,results,fees}){
  ...
  const [term,setTerm]=useState("Annual Exam 2026");
...
<div class="info">
  <div class="info-row"><span class="lbl">Name / Name:</span><span class="val">${selStudent?.name||"—"}</span></div>
  <div class="info-row"><span class="lbl">Father / Father:</span><span class="val">${selStudent?.fatherName||"—"}</span></div>
  <div class="info-row"><span class="lbl">Admission Marks / Adm:</span><span class="val">${selStudent?.studentCode||"—"}</span></div>
  <div class="info-row"><span class="lbl">Grade / Class:</span><span class="val">${selStudent?.grade||"—"}</span></div>
  <div class="info-row"><span class="lbl">Section / Section:</span><span class="val">${selStudent?.section||"—"}</span></div>
  <div class="info-row"><span class="lbl">Date / Date:</span><span class="val">${new Date().toLocaleDateString("en-PK")}</span></div>
</div>
${(()=>{
  ...
  const rows=sR.map((r,i)=>{
    ...
    return `<tr><td>${i+1}</td><td><strong>${r.subject}</strong></td><td>${r.total||100}</td><td style="color:${p>=40?"#1e293b":"#dc2626"};font-weight:700">${r.obtained||0}</td><td style="color:${p>=70?"#16a34a":p>=50?"#d97706":"#dc2626"};font-weight:700">${p}%</td><td>${r.grade||"—"}</td><td class="${p>=40?"pass":"fail"}">${p>=40?"✅ Pass":"❌ Fail"}</td></tr>`;
  }).join("");
  const remarks=pct>=80?"MashaAllah! Student showed excellent performance.":pct>=60?"Student performance is satisfactory.":pct>=40?"Student needs more effort.":"Student failed. Immediate attention required.";
  return `<div class="summary">
    <div class="sum-box"><div class="sum-big" style="color:${pct>=50?"#16a34a":"#dc2626"}">${grade}</div><div class="sum-lbl">Total Grade</div></div>
    <div class="sum-box"><div class="sum-big" style="color:${pct>=50?"#16a34a":"#dc2626"}">${pct}%</div><div class="sum-lbl">Percentage</div></div>
    <div class="sum-box"><div class="sum-big">${totO}/${totM}</div><div class="sum-lbl">Marks</div></div>
    <div class="sum-box"><div class="sum-big" style="color:${passed?"#16a34a":"#dc2626"}">${passed?"پاس":"فیل"}</div><div class="sum-lbl">نتیجہ</div></div>
  </div>
  <table><thead><tr><th>#</th><th>Subject</th><th>Total</th><th>Earned</th><th>Percentage</th><th>Grade</th><th>Result</th></tr></thead>
  <tbody>${rows||"<tr><td colspan='7' style='text-align:center;color:#bbb;padding:20px'>کوئی نتائج درج نہیں</td></tr>"}</tbody>
  <tfoot><tr><td colspan="2">Total</td><td>${totM}</td><td>${totO}</td><td>${pct}%</td><td>${grade}</td><td class="${passed?"pass":"fail"}">${passed?"✅ Pass":"❌ Fail"}</td></tr></tfoot></table>
  <div class="remarks">📝 Remarks: ${remarks}</div>`;
})()}
<div class="sig">
  <div class="sig-box"><div class="sig-line"></div><div class="sig-lbl">Class Teacher / Class Teacher</div></div>
  <div class="sig-box"><div class="sig-line"></div><div class="sig-lbl">Principal / Principal</div></div>
  <div class="sig-box"><div class="sig-line"></div><div class="sig-lbl">Parents</div></div>
</div>
<div class="footer">AMEEN ISLAMIC INSTITUTE • SWAT • This certificate is not valid without seal and signature</div>
<script>window.onload=function(){window.print();setTimeout(function(){window.close();},2000);};<\/script>
  ...
  if(selStudent){
    ...
    return <div style={S.page}>
      <div style={{display:"flex",gap:"10px",marginBottom:"20px",flexWrap:"wrap"}}>
        ...
        <button style={{...S.saveBtn,fontSize:"0.65rem"}} onClick={printDMC}>🖨️ Print / Download DMC</button>
      </div>
      <div id="dmc-print-area" style={{background:C.white,borderRadius:"20px",overflow:"hidden",boxShadow:"0 8px 32px rgba(0,0,0,0.12)",maxWidth:"820px",margin:"0 auto",padding:"24px 28px",border:`3px solid ${C.gold}40`}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"6px 24px",padding:"12px 16px",background:"#fafaf8",borderRadius:"10px",border:`1px solid ${C.goldLight}`,marginBottom:"16px"}}>
          {[["Name / Name",selStudent.name],["Father / Father",selStudent.fatherName||"—"],["Admission Marks",selStudent.studentCode||"—"],["Grade / Class",selStudent.grade||"—"],["Section",selStudent.section||"—"],["House",`${h.emoji||""} ${h.nameEn||"—"}`]].map(([l,v])=>(
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"10px",marginBottom:"16px"}}>
          {[{icon:"🏆",label:"Grade",val:overallGrade,color:gradeColor},{icon:"📊",label:"Percentage",val:`${avgPct}%`,color:avgPct>=50?C.green:C.red},{icon:"📝",label:"Marks",val:`${totalObtained}/${totalMarks}`,color:C.navy},{icon:passed?"✅":"❌",label:"نتیجہ",val:passed?"پاس":"فیل",color:passed?C.green:C.red}].map((item,i)=>(
        ...
        <div style={{fontSize:"0.75rem",fontWeight:"800",color:C.navy,marginBottom:"10px"}}>📋 Subject-wise Marks</div>
        <table style={{width:"100%",borderCollapse:"collapse",marginBottom:"16px",fontSize:"0.67rem"}}>
          <thead><tr style={{background:`linear-gradient(135deg,${C.navy},${C.navyMid})`,color:C.white}}>{["#","Subject","Total","Earned","Percentage","Grade","Result"].map(h=><th key={h} style={{padding:"8px 10px",textAlign:"left",fontWeight:"700"}}>{h}</th>)}</tr></thead>
          <tbody>{sResults.map((r,i)=>{const p=r.percentage||Math.round(((r.obtained||0)/(r.total||100))*100);return <tr key={r.id} style={{background:i%2===0?"#fafaf8":C.white}}>
            ...
            <td style={S.td}><span style={{fontWeight:"700",color:p>=40?C.green:C.red,fontSize:"0.62rem"}}>{p>=40?"✅ Pass":"❌ Fail"}</span></td>
          </tr>;})}
          {sResults.length===0&&<tr><td colSpan={7} style={{...S.td,textAlign:"center",color:"#bbb",padding:"30px"}}><span className="ur">کوئی نتائج درج نہیں</span></td></tr>}
          </tbody>
          {sResults.length>0&&<tfoot><tr style={{background:C.goldLight}}>
            <td colSpan={2} style={{...S.td,fontWeight:"900",color:C.navy}}>Total</td>
            ...
            <td style={S.td}><span style={{fontWeight:"900",color:passed?C.green:C.red}}>{passed?"✅ Pass":"❌ Fail"}</span></td>
          </tr></tfoot>}
        </table>
        {pendingFees>0&&<div style={{background:"#fff7ed",border:`2px solid ${C.amber}30`,borderRadius:"10px",padding:"10px 14px",marginBottom:"14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <span style={{fontSize:"0.68rem",color:C.amber,fontWeight:"700"}}>⚠️ Fee Pending</span>
        ...
        <div style={{background:`linear-gradient(135deg,${C.goldLight},#fdf8ee)`,borderRadius:"10px",padding:"12px 16px",marginBottom:"20px",border:`1px solid ${C.gold}30`}}>
          <div style={{fontSize:"0.65rem",fontWeight:"800",color:C.navy,marginBottom:"4px"}}>📝 Remarks</div>
          <div style={{fontSize:"0.63rem",color:C.navy,lineHeight:"1.9"}}>{avgPct>=80?"MashaAllah! Student showed excellent performance.":avgPct>=60?"Student performance is satisfactory.":avgPct>=40?"Student needs more effort.":"Student failed."}</div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"24px",paddingTop:"16px"}}>
          {["Class Teacher","Principal","Parents"].map(r=>(
        ...
        <div style={{marginTop:"16px",paddingTop:"10px",borderTop:`2px double ${C.gold}`,textAlign:"center"}}>
          <div style={{fontFamily:"'Cinzel',serif",fontSize:"0.55rem",color:"#bbb",letterSpacing:"0.15em"}}>AMEEN ISLAMIC INSTITUTE • SWAT • This certificate is not valid without seal and signature</div>
        </div>
      </div>
    </div>;
  }
  ...
  return <div style={S.page}>
    <div style={{fontSize:"1.1rem",fontWeight:"700",color:C.navy,marginBottom:"4px"}}>🎓 Detail Marks Certificate (DMC)</div>
    <div style={{fontSize:"0.62rem",color:"#888",marginBottom:"16px"}}>Detail Marks Certificate — Student Select</div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px",marginBottom:"20px"}}>
      <input style={{...S.inpSm,fontSize:"0.8rem",padding:"14px 18px"}} placeholder="🔍 Search by name or admission no..." value={q} onChange={e=>setQ(e.target.value)}/>
    ...
    {q&&<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:"14px"}}>
      ...
      {filtered.length===0&&<div className="hv-card ur" style={{...S.card,textAlign:"center",color:"#bbb",padding:"40px",gridColumn:"1/-1"}}><div style={{fontSize:"2rem",marginBottom:"8px"}}>🔍</div>کوئی طالب علم نہیں ملا</div>}
    </div>}
    {!q&&<div className="hv-card" style={{...S.card,textAlign:"center",padding:"60px"}}>
      <div style={{fontSize:"3rem",marginBottom:"12px"}}>🎓</div>
      <div style={{fontSize:"0.85rem",fontWeight:"700",color:C.navy}}>Detail Marks Certificate (DMC)</div>
      <div style={{fontSize:"0.62rem",color:"#888",marginTop:"8px",lineHeight:"1.8"}}>Detail Marks Certificate<br/>Search student by name Write</div>
    </div>}
  </div>;
}
