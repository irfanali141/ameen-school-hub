...
function Events({addData,houses,updateHousePoints}){
  ...
  const evTypes={"competition":{l:"مقابلہ",icon:"emoji_events",color:"#f59e0b"},"sports":{l:"کھیل",icon:"sports_soccer",color:"#22c55e"},"academic":{l:"تعلیمی",icon:"school",color:"#60a5fa"},"cultural":{l:"ثقافتی",icon:"theater_comedy",color:"#a78bfa"},"service":{l:"سماجی",icon:"volunteer_activism",color:"#34d399"}};
    ...
    <div style={{minHeight:"100vh",background:`linear-gradient(160deg,${N} 0%,#0d1f3c 50%,#0a1628 100%)`,padding:"24px 20px",fontFamily:"'Public Sans',sans-serif",direction:"rtl"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"28px",flexWrap:"wrap",gap:"12px"}}>
        <div style={{display:"flex",alignItems:"center",gap:"12px"}}>
          ...
          <div>
            <h1 style={{margin:0,fontSize:"1.5rem",fontWeight:"800",color:"#f1f5f9"}}>ایونٹس</h1>
            <p style={{margin:0,fontSize:"0.75rem",color:"rgba(212,175,55,0.7)",fontWeight:"500"}}>Events & Competitions • {events.length} ایونٹ</p>
          </div>
        </div>
        <button onClick={()=>setShow(!show)} style={{display:"flex",alignItems:"center",gap:"8px",padding:"11px 22px",borderRadius:"12px",border:`1px solid ${G}`,background:show?"rgba(212,175,55,0.15)":"transparent",color:G,fontWeight:"700",fontSize:"0.85rem",cursor:"pointer",fontFamily:"'Public Sans',sans-serif"}}>
          <span className="material-symbols-rounded" style={{fontSize:"20px"}}>{show?"close":"add_circle"}</span>{show?"منسوخ":"نیا ایونٹ"}
        </button>
      </div>
      {/* Add Form */}
      {show&&(
        <div style={{...glass,padding:"24px",marginBottom:"24px",borderColor:"rgba(212,175,55,0.3)"}}>
          <div style={{display:"flex",alignItems:"center",gap:"10px",marginBottom:"20px"}}>
            ...
            <span style={{color:G,fontWeight:"700",fontSize:"1rem"}}>نئے ایونٹ کا اندراج</span>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"14px",marginBottom:"16px"}}>
            <div style={{gridColumn:"1/-1"}}><label style={lbl}>ایونٹ کا نام *</label><input style={inp} value={f.name} onChange={e=>setF({...f,name:e.target.value})} placeholder="نعت مقابلہ..."/></div>
            ...
            <div><label style={lbl}>فاتح ہاؤس</label><select style={inp} value={f.houseId} onChange={e=>setF({...f,houseId:e.target.value})}>{HOUSES.map(h=><option key={h.id} value={h.id} style={{background:N2}}>{h.emoji} {h.nameEn}</option>)}</select></div>
            <div><label style={lbl}>پوائنٹس</label><input style={{...inp,direction:"ltr"}} type="number" value={f.points} onChange={e=>setF({...f,points:e.target.value})}/></div>
            <div style={{gridColumn:"1/-1"}}><label style={lbl}>نوٹس</label><input style={inp} value={f.notes} onChange={e=>setF({...f,notes:e.target.value})} placeholder="تفصیل..."/></div>
          </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:"16px"}}>
          ...
          <div key={ev.id} className="hv-card" style={{...glass,padding:"20px",borderRight:`3px solid ${h.color||G}`,position:"relative",overflow:"hidden"}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"12px"}}>
              ...
              <div style={{background:`linear-gradient(135deg,${G},#b8960a)`,borderRadius:"10px",padding:"6px 12px",textAlign:"center",flexShrink:0}}>
                <div style={{fontSize:"0.6rem",color:N,fontWeight:"700",marginBottom:"1px"}}>پوائنٹس</div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:"8px",padding:"8px 12px",borderRadius:"10px",background:`${h.color||G}15`,border:`1px solid ${h.color||G}30`}}>
              ...
              <span style={{fontSize:"0.72rem",fontWeight:"600",color:h.color||G}}>فاتح: {h.nameEn||"—"}</span>
            </div>
        ...
        {events.length===0&&(
          <div style={{...glass,padding:"60px 20px",textAlign:"center",gridColumn:"1/-1"}}>
            ...
            <div style={{color:"rgba(241,245,249,0.4)",fontSize:"0.9rem"}}>ابھی کوئی ایونٹ نہیں</div>
          </div>
        )}
      </div>
    </div>
